...
function postCardTitle(item) {
  const postRef = item?.post_ref || {};
  return postRef.title || 'Публикация Friendscape';
}

function postCardSubtitle(item) {
  const postRef = item?.post_ref || {};
  return postRef.text || postRef.author_name || 'Открывает публикацию в ленте';
}

function buildPostHref(postRef = {}) {
  if (postRef.deep_link) return postRef.deep_link;
  if (postRef.post_id) return `/feed?post=${postRef.post_id}`;
  return postRef.url || '';
}

function stripTextPart(text, part) {
  return part ? text.split(part).join('') : text;
}

function cleanSharedPostMessageText(item) {
  const raw = String(item?.text || '').trim();
  if (!raw || !item?.post_ref || raw === 'Публикация') return raw === 'Публикация' ? '' : raw;

  const postRef = item.post_ref || {};
  let next = raw.replace(/https?:\/\/\S+/g, '').trim();
  next = stripTextPart(next, postRef.url);
  next = stripTextPart(next, postRef.deep_link);
  next = stripTextPart(next, postRef.post_id ? `/feed?post=${postRef.post_id}` : '');
...
function getPostTypeLabel(type = 'post') {
  ...
  if (normalized === 'gallery') return 'Фото';
  if (normalized === 'video') return 'Видео';
  if (normalized === 'link') return 'Ссылка';
  if (normalized === 'repost') return 'Репост';
  return 'Пост';
}

function getPostAuthorName(post, postRef = {}) {
  if (postRef?.author_name) return postRef.author_name;
  const author = post?.author || {};
  return `${author.first_name || ''} ${author.last_name || ''}`.trim() || 'Автор публикации';
}

function getPostPayload(post, postRef = {}) {
  return post?.payload && typeof post.payload === 'object' ? post.payload : (postRef?.payload && typeof postRef.payload === 'object' ? postRef.payload : {});
}

function getPostPreviewTitle(post, postRef = {}) {
  const payload = getPostPayload(post, postRef);
  return String(payload.title || payload.innerTitle || postRef.title || 'Публикация Friendscape').trim();
}

function getPostPreviewText(post, postRef = {}) {
  const payload = getPostPayload(post, postRef);
  return String(post?.text || postRef.text || payload.desc || payload.innerDesc || '').trim();
}

function getRealtimeStatusMeta(status) {
  const normalized = String(status || '').toLowerCase();
  if (normalized === 'reconnecting') return { visible: true, className: 'is-warning', label: 'Подключение…' };
  if (normalized === 'syncing') return { visible: true, className: 'is-info', label: 'Синхронизация' };
  if (['error', 'failed', 'offline'].includes(normalized)) return { visible: true, className: 'is-warning', label: 'Связь нестабильна' };
  return { visible: false, className: 'is-success', label: '' };
}

function PostReferenceIcon() {
  return <svg viewBox="0 0 24 24"><path d="M5 4h14a1 1 0 0 1 1 1v14a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1z" /><path d="M8 8h8" /><path d="M8 12h8" /><path d="M8 16h5" /></svg>;
}
...
function MessageFileCard({ item, messageSelectionMode = false }) {
  ...
  const mediaUrl = sanitizeUrlForClient(item?.media?.url);
  ...
  return (
    <a
      ...
      rel="noreferrer"
      target="_blank"
      onClick={(event) => { event.stopPropagation(); if (!mediaUrl) event.preventDefault(); }}
...
function PostReferenceCard({ item, messageSelectionMode = false, onOpenPostPreview }) {
  const postRef = item?.post_ref || {};
  const href = buildPostHref(postRef);
  const disabled = Boolean(messageSelectionMode);

  const handleOpen = (event) => {
    event.stopPropagation();
    if (disabled) return;
    if (onOpenPostPreview) {
      onOpenPostPreview(postRef);
      return;
    }
  ...
  return (
    <button
      type="button"
      className={`chatW-postRefCard ${item.direction === 'outgoing' ? 'is-outgoing' : 'is-incoming'} ${disabled ? 'is-disabled' : ''}`.trim()}
      onPointerDown={(event) => event.stopPropagation()}
      onClick={handleOpen}
      disabled={disabled}
    >
      <div className="chatW-postRefIcon" aria-hidden="true"><PostReferenceIcon /></div>
      <div className="chatW-postRefBody">
        <div className="chatW-postRefTopline">
          <strong className="chatW-postRefTitle">{postCardTitle(item)}</strong>
          <span className="chatW-postRefPill">{getPostTypeLabel(postRef.type)}</span>
        </div>
        <div className="chatW-postRefText">{postCardSubtitle(item)}</div>
        <div className="chatW-postRefFooter">
          <span>{disabled ? 'Вложение публикации' : 'Открыть карточку'}</span>
          {postRef.author_name ? <small>{postRef.author_name}</small> : null}
        </div>
      </div>
    </button>
  );
...
function SharedPostPreviewSheet({ state, onClose, onOpenInFeed }) {
  if (!state?.open) return null;
  const postRef = state.postRef || {};
  const post = state.post || null;
  const payload = getPostPayload(post, postRef);
  const title = getPostPreviewTitle(post, postRef);
  const text = getPostPreviewText(post, postRef);
  const authorName = getPostAuthorName(post, postRef);
  const postType = post?.type || postRef.type || 'post';
  const stats = post?.stats || {};
  const slides = Array.isArray(payload.slides) ? payload.slides : [];
  const hasStats = Boolean(stats.plus || stats.minus || stats.comments || stats.saves || stats.reposts || stats.views);

  return (
    <div className="chatW-postPreviewBackdrop" onClick={onClose}>
      <div className="chatW-postPreviewSheet" role="dialog" aria-modal="true" aria-label="Карточка публикации" onClick={(event) => event.stopPropagation()}>
        <div className="chatW-sheetHandle" aria-hidden="true" />
        <div className="chatW-postPreviewHead">
          <div>
            <div className="chatW-postPreviewEyebrow">{getPostTypeLabel(postType)} из ленты</div>
            <div className="chatW-postPreviewTitle">{title}</div>
          </div>
          <button type="button" className="chatW-postPreviewClose" onClick={onClose} aria-label="Закрыть">×</button>
        </div>
        {state.loading ? <div className="chatW-postPreviewState">Загружаю публикацию…</div> : state.error ? <div className="chatW-postPreviewState is-error">{state.error}</div> : null}
        <article className="chatW-postPreviewCard">
          <header className="chatW-postPreviewAuthor">
            <div className="chatW-postPreviewAvatar" aria-hidden="true">{authorName.slice(0, 2).toUpperCase()}</div>
            <div><strong>{authorName}</strong><span>{payload.meta || post?.location || 'Публикация Friendscape'}</span></div>
          </header>
          {text ? <p className="chatW-postPreviewText">{text}</p> : null}
          {postType === 'gallery' && slides.length ? (
            <div className="chatW-postPreviewGallery">
              {slides.slice(0, 3).map((slide, index) => (
                <div key={`${slide.text || 'slide'}-${index}`} className="chatW-postPreviewSlide" style={slide.bg ? { background: slide.bg } : undefined}><span>{slide.text || `Слайд ${index + 1}`}</span></div>
              ))}
            </div>
          ) : null}
          {postType === 'video' ? <div className="chatW-postPreviewMedia is-video"><span className="chatW-postPreviewPlay">▶</span><strong>{payload.title || title}</strong>{payload.desc ? <small>{payload.desc}</small> : null}</div> : null}
          {postType === 'link' ? <div className="chatW-postPreviewLink">{payload.domain ? <span>{payload.domain}</span> : null}<strong>{payload.title || title}</strong>{payload.desc ? <small>{payload.desc}</small> : null}</div> : null}
          {postType === 'repost' ? <div className="chatW-postPreviewRepost"><strong>{payload.innerTitle || 'Оригинальный пост'}</strong><span>{payload.innerDesc || payload.desc || 'Вложенная публикация'}</span></div> : null}
          {hasStats ? <footer className="chatW-postPreviewStats"><span>＋ {stats.plus || 0}</span><span>− {stats.minus || 0}</span><span>💬 {stats.comments || 0}</span><span>↗ {stats.reposts || 0}</span>{stats.views ? <span>👁 {stats.views}</span> : null}</footer> : null}
        </article>
        <div className="chatW-postPreviewActions"><button type="button" className="chatW-postPreviewSecondary" onClick={onClose}>Закрыть</button><button type="button" className="chatW-postPreviewPrimary" onClick={() => onOpenInFeed(postRef)}>Открыть в ленте</button></div>
      </div>
    </div>
  );
}
...
function TimelineMessageItem(props) {
  ...
  const displayText = cleanSharedPostMessageText(item);
  ...
  return (
    <div className={`chatW-row ${item.direction === 'outgoing' ? 'is-outgoing' : 'is-incoming'} ${messageSelectionMode ? 'is-selection-mode' : ''}`}>
      ...
      >
        ...
        {item.post_ref ? (
          <PostReferenceCard item={item} messageSelectionMode={messageSelectionMode} onOpenPostPreview={onOpenPostPreview} />
        ) : null}
        ...
        {displayText ? <div className="chatW-bubble-text">{displayText}</div> : null}
        {!displayText && !item.media && item.preview_text && !item.post_ref ? <div className="chatW-bubble-text">{item.preview_text}</div> : null}
...
export default function ChatConversationWorkspace({
  ...
  const [sharedPostPreview, setSharedPostPreview] = useState({ open: false, loading: false, error: '', postRef: null, post: null });
  ...
  const openSharedPostPreview = async (postRef = {}) => {
    const normalizedPostRef = postRef && typeof postRef === 'object' ? postRef : {};
    const postId = Number(normalizedPostRef.post_id || normalizedPostRef.postId || 0);
    setSharedPostPreview({ open: true, loading: Boolean(postId), error: '', postRef: normalizedPostRef, post: null });
    if (!postId) return;
    try {
      const response = await fetch(`/api/feed/posts/${postId}`, { cache: 'no-store' });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data?.error || 'Не удалось загрузить публикацию.');
      setSharedPostPreview({ open: true, loading: false, error: '', postRef: normalizedPostRef, post: data.post || null });
    } catch (error) {
      setSharedPostPreview((current) => ({ ...current, loading: false, error: error?.message || 'Не удалось загрузить публикацию.' }));
    }
  };

  const closeSharedPostPreview = () => setSharedPostPreview({ open: false, loading: false, error: '', postRef: null, post: null });

  const openSharedPostInFeed = (postRef = {}) => {
    const href = buildPostHref(postRef);
    if (href && typeof window !== 'undefined') window.location.assign(href);
  };
  ...
  const realtimeStatusMeta = getRealtimeStatusMeta(realtimeState?.status);
  ...
  return (
    <section className={`chatW-dialog ${showList ? 'is-mobile-hidden' : 'is-mobile-visible'}`}>
      ...
      <div className="chatW-timeline" ref={timelineRef}>
        ...
        {realtimeStatusMeta.visible ? (
          <div className="chatW-stateRow">
            <span className={`chatW-flag ${realtimeStatusMeta.className}`}>
        ...
        {(showTimelineSkeleton ? [] : timelineItems).map((item) => {
          ...
          return (
            <TimelineMessageItem
              ...
              onOpenPostPreview={openSharedPostPreview}
      ...
      <SharedPostPreviewSheet
        state={sharedPostPreview}
        onClose={closeSharedPostPreview}
        onOpenInFeed={openSharedPostInFeed}
      />
          ...
          <div className="chatW-sheetPanel chatW-infoPanel" role="dialog" aria-modal="true" aria-label="Информация о диалоге" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="chatW-infoPanelHero">
              ...
              <div className="chatW-infoPanelHeroCopy">
                ...
                <span>{compactHeaderStatus || 'Статус не указан'}</span>
              </div>
            </div>
              ...
              {mediaHubFilteredItems.map((entry) => (
                ...
                >
                  <span className={`chatW-mediaHubThumb is-${entry.kind}`}>
                    ...
                    {entry.kind === 'video' ? (entry.thumbUrl ? <img src={entry.thumbUrl} alt="" loading="lazy" draggable={false} /> : entry.url ? <video src={entry.url} muted playsInline preload="metadata" /> : <span>▶</span>) : null}
