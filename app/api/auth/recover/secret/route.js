...
import { revokeUserSessionsForRecovery } from '@/lib/recovery';
...
export async function POST(request) {
  try {
    ...
    const recoveryLimit = await enforceRateLimit({
      ...
      subject: `recover:secret:${getClientIp(request)}:${normalizedKey(firstName, lastName) || 'missing'}`,
      metadata: { method: 'secret_answer' },
    });
