...
export async function POST(request) {
  try {
    ...
    const loginLimit = await enforceRateLimit({
      request,
      policy: 'auth_login',
      subject: `login:${getClientIp(request)}:${normalizedKey(firstName, lastName) || 'missing'}`,
    ...
    if (user.accountStatus === 'deleted') {
      await writeAuditLog({ request, actorUserId: user.id, action: 'auth.login', status: 'failed', metadata: { reason: 'account_deleted' } });
      return NextResponse.json({ error: 'Аккаунт был удалён.' }, { status: 410 });
    }
