...
import { cleanupExpiredRecoverySessionById, createRecoveryCompletionToken, getAvailableRecoveryMethods, hashRecoveryCompletionToken, verifyRecoveryMethod } from '@/lib/recovery';
...
export async function POST(request) {
  try {
    ...
    const recoveryLimit = await enforceRateLimit({
      ...
      subject: `recovery-submit:${getClientIp(request)}:${recoveryId || 'missing'}`,
      metadata: { method },
    });
    ...
    const verification = await verifyRecoveryMethod(recovery.user, method, body, { request });
    if (!verification.ok) {
    ...
    if (method === 'trusted_device' && verification.device) {
      nextMetadata.trusted_device = verification.device;
    }

    const completionToken = createRecoveryCompletionToken();
    nextMetadata.completion_token_hash = hashRecoveryCompletionToken(completionToken);
    nextMetadata.completion_token_created_at = new Date().toISOString();
