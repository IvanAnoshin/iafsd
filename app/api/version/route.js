...
async function readPackageMetadata() {
  try {
    ...
    logError('version.metadata_read_failed', error, { route: '/api/version' });
...
async function buildVersionPayload() {
  ...
  return {
    ...
    app: {
      ...
      releaseChannel: process.env.APP_RELEASE_CHANNEL || 'local',
      versionTag: process.env.APP_VERSION_TAG || null,
    },
    build: {
      commitSha,
      buildTime,
...
export async function HEAD(request) {
  return withApiMonitoring(request, async ({ requestId }) => new NextResponse(null, {
    status: 200,
