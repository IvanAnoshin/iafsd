...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    const report = await updateAdminMessageReportStatus(resolvedParams.id, body?.status || body?.global_status);
    ...
    return NextResponse.json({ report, message: 'Статус жалобы на сообщение обновлён.' }, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
