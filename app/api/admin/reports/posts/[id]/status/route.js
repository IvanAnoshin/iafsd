...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    const report = await updateAdminPostReportStatus(resolvedParams.id, body?.status || body?.global_status);

    await writeAuditLog({ request, session, action: 'admin.post_report.status', entityType: 'post_report', entityId: resolvedParams.id, metadata: { status: report.status } });
    ...
    await writeAuditLog({ request, session, action: 'admin.post_report.status', entityType: 'post_report', entityId: resolvedParams?.id, status: 'error', metadata: { message: error?.message || 'unknown_error' } });
