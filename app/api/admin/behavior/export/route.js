...
import { adminCsv, adminJson, requireAdminRequest } from '@/lib/admin-security';
import { writeAuditLog } from '@/lib/audit';

export async function GET(request) {
  let auditSession = null;
  try {
    const guard = await requireAdminRequest(request, { action: 'admin.behavior.export', exportAction: true });
    ...
    return adminJson({ error: 'Не удалось выгрузить DFSN-датасет.' }, { status: 500 });
  }
}
