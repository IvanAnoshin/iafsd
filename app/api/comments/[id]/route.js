...
import { loadCommentForViewer } from '@/lib/access-control';
...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    const existing = await loadCommentForViewer(commentId, session.user.id, prisma);
    ...
    return NextResponse.json({ error: error?.status && error.status < 500 ? error.message : 'Не удалось обновить комментарий.' }, { status: error?.status || 500 });
  }
}
...
export async function DELETE(request, { params }) {
  ...
  try {
    ...
    return NextResponse.json({ error: error?.status && error.status < 500 ? error.message : 'Не удалось удалить комментарий.' }, { status: error?.status || 500 });
  }
}
