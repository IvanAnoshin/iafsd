...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const comment = await loadCommentForViewer(commentId, session.user.id, prisma);
    ...
    return NextResponse.json({ error: error?.status && error.status < 500 ? error.message : 'Не удалось обновить голос комментария.' }, { status: error?.status || 500 });
  }
}
