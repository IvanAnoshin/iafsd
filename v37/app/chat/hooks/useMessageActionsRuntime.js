...
export function useMessageActionsRuntime({
  ...
  const handleCopyMessage = useCallback(async () => {
    ...
    try {
      ...
      setErrorText('Текст сообщения скопирован.');
      setSelectedMessageId(null);
    } catch (error) {
  ...
  const handleToggleSaveMessage = useCallback(async () => {
    ...
    try {
      ...
      setErrorText(payload.message.is_saved ? 'Сообщение сохранено.' : 'Сообщение убрано из сохранённых.');
      setSelectedMessageId(null);
    } catch (error) {
  ...
  const handleReportSelectedMessage = useCallback(async () => {
    ...
    const reason = window.prompt('Укажи причину жалобы (например: спам, оскорбление, мошенничество, угрозы).', 'спам');
    ...
    const details = window.prompt('Добавить комментарий к жалобе? Это необязательно.', '');
    ...
    const blockFutureMessages = canBlockPeer
      ? window.confirm('После жалобы сразу ограничить будущие входящие сообщения от этого пользователя в этом диалоге?')
      : false;
    setMessageActionLoading(true);
    try {
      ...
      setErrorText(
        payload?.blocked_peer
          ? 'Жалоба отправлена. Будущие входящие сообщения от пользователя в этом диалоге ограничены.'
          : (payload?.safety_flagged ? 'Жалоба отправлена. Сообщение помечено для safety-проверки.' : 'Жалоба отправлена.'),
      );
      setSelectedMessageId(null);
    } catch (error) {
  ...
  }, [activeChatId, conversationMeta?.type, loadRequests, mergeMessages, readJsonSafe, selectedMessage, setChats, setConversationMeta, setErrorText, setMessages, usingFallback]);
  ...
  const handleTogglePinMessage = useCallback(async () => {
    ...
    try {
      ...
      setErrorText(payload.message.is_pinned ? 'Сообщение закреплено.' : 'Сообщение откреплено.');
      setSelectedMessageId(null);
    } catch (error) {
