...
import { extendStoryFoundation } from '@/lib/stories';
...
export async function POST(request, context) {
  try {
    ...
    const story = await extendStoryFoundation(session.user.id, storyId, body);
    return NextResponse.json({ ok: true, story }, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    console.error('story extend failed', error);
    return NextResponse.json({ error: error?.message || 'Не удалось продлить момент.' }, { status: error?.status || 500 });
  }
}
