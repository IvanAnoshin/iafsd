...
const DEMO_PASSWORD = 'demo-user-password';
const DEMO_SECRET = 'demo-secret-answer';
const DEMO_CODES = ['111-111', '222-222', '333-333'];

const demoPeople = [
  { firstName: 'Анна', lastName: 'Смирнова', handle: 'anna.smirnova', occupation: 'UI designer', city: 'Вильнюс', tone: 'mint', status: 'online', mutualHint: 12 },
  { firstName: 'Илья', lastName: 'Кузнецов', handle: 'ilya.kuznetsov', occupation: 'Frontend developer', city: 'Рига', tone: 'blue', status: 'recent', mutualHint: 7 },
  { firstName: 'Мария', lastName: 'Орлова', handle: 'maria.orlova', occupation: 'Иллюстратор', city: 'Каунас', tone: 'gold', status: 'online', mutualHint: 3 },
  { firstName: 'Даниил', lastName: 'Миронов', handle: 'dan.mironov', occupation: 'Product designer', city: 'Варшава', tone: 'rose', status: 'recent', mutualHint: 18 },
  { firstName: 'Вера', lastName: 'Павлова', handle: 'vera.pavlova', occupation: 'Фотограф', city: 'Таллин', tone: 'violet', status: 'recent', mutualHint: 2 },
  { firstName: 'Леонард', lastName: 'Дайвинчик', handle: 'leo.day', occupation: 'Музыкальный редактор', city: 'Вильнюс', tone: 'slate', status: 'online', mutualHint: 9 },
];
...
function buildNormalizedKey(firstName, lastName) {
...
async function ensureDemoUsers(currentUser) {
  const passwordHash = await bcrypt.hash(DEMO_PASSWORD, 6);
  const secretAnswerHash = await bcrypt.hash(DEMO_SECRET, 6);
  const backupCodeHashes = await Promise.all(DEMO_CODES.map((item) => bcrypt.hash(item, 4)));

  await ensureProfileForUser(currentUser);

  const createdUsers = [];
  for (const demo of demoPeople) {
    const normalizedKey = buildNormalizedKey(demo.firstName, demo.lastName);
    let user = await prisma.user.findUnique({ where: { normalizedKey } });
    if (!user) {
    ...
    const existingProfile = await prisma.userPublicProfile.findUnique({ where: { userId: user.id } });
    if (!existingProfile) {
      await prisma.userPublicProfile.create({
        data: {
          userId: user.id,
          handle: demo.handle,
  ...
  const [anna, ilya, maria, daniil, vera, leonard] = createdUsers;

  const existingGraphCount = await prisma.$transaction([
    prisma.friendship.count({
      where: { OR: [{ userAId: currentUser.id }, { userBId: currentUser.id }] },
    }),
    prisma.friendRequest.count({
      where: { OR: [{ fromUserId: currentUser.id }, { toUserId: currentUser.id }] },
    }),
  ...
  if (existingGraphCount.some((item) => item > 0)) return;

  const friendshipSeeds = [
    [currentUser.id, anna.id],
    [currentUser.id, ilya.id],
  ];

  for (const [left, right] of friendshipSeeds) {
    const [userAId, userBId] = sortFriendPair(left, right);
  ...
  const incomingSeeds = [maria.id, daniil.id];
  for (const fromUserId of incomingSeeds) {
    await prisma.friendRequest.upsert({
      where: { fromUserId_toUserId: { fromUserId, toUserId: currentUser.id } },
      update: { status: 'pending' },
      create: { fromUserId, toUserId: currentUser.id, status: 'pending' },
    });
  }

  await prisma.friendRequest.upsert({
    where: { fromUserId_toUserId: { fromUserId: currentUser.id, toUserId: vera.id } },
    update: { status: 'pending' },
    create: { fromUserId: currentUser.id, toUserId: vera.id, status: 'pending' },
  });

  const subscriptionSeeds = [
    [currentUser.id, maria.id],
    [currentUser.id, leonard.id],
    [anna.id, currentUser.id],
    [vera.id, currentUser.id],
  ];

  for (const [fromUserId, toUserId] of subscriptionSeeds) {
...
async function buildPeoplePayload(currentUser, query, filter, sort = 'relevant') {
  await ensureDemoUsers(currentUser);
...
export async function GET(request) {
  try {
    ...
    console.error('people/get failed', error);
