...
function normalizeMediaPayload(payload = {}, type = 'text') {
  ...
  const mediaUrl = toBoundedString(source.url || source.mediaUrl || source.path || '', 4000);
  const mediaThumbUrl = toBoundedString(source.thumbUrl || source.thumbnailUrl || '', 4000);
...
function normalizeStoryReference(payload = {}, type = 'text') {
  ...
  const previewUrl = toBoundedString(source.preview_url || source.previewUrl || source.thumb_url || source.thumbUrl || source.cover_url || source.coverUrl || source.image_url || source.imageUrl || '', 4000) || null;
  const deepLink = toBoundedString(source.deep_link || source.deepLink || source.url || source.href || '', 4000) || null;
