...
function buildSeedMediaDataUri(user, index = 0, kind = 'photo') {
  const palette = [
    ['#1b2452', '#2f4fb8', '#8ea2ff'],
    ['#2a163f', '#7b3db8', '#f39ecf'],
    ['#112f2d', '#1c8f77', '#9fe4d5'],
    ['#32210f', '#b8731f', '#f8d38f'],
  ][index % 4];
  const title = (kind === 'video' ? 'Видео момент' : 'Момент').replace(/&/g, '&amp;');
  const name = buildAuthorLabel(user).replace(/&/g, '&amp;');
  const handle = clip(user?.publicProfile?.handle || '', 40).replace(/&/g, '&amp;');
  const svg = `
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1080 1920">
    <defs>
      <linearGradient id="g" x1="0" x2="1" y1="0" y2="1">
        <stop stop-color="${palette[0]}" offset="0%"/>
        <stop stop-color="${palette[1]}" offset="55%"/>
        <stop stop-color="${palette[2]}" offset="100%"/>
      </linearGradient>
    </defs>
    <rect width="1080" height="1920" rx="0" fill="url(#g)"/>
    <circle cx="850" cy="320" r="220" fill="rgba(255,255,255,0.12)"/>
    <circle cx="220" cy="1450" r="280" fill="rgba(255,255,255,0.09)"/>
    <rect x="88" y="88" width="904" height="1744" rx="54" fill="rgba(8,10,16,0.16)" stroke="rgba(255,255,255,0.18)"/>
    <text x="140" y="270" fill="rgba(255,255,255,0.92)" font-size="72" font-family="Arial, sans-serif" font-weight="700">${name}</text>
    <text x="140" y="350" fill="rgba(255,255,255,0.72)" font-size="42" font-family="Arial, sans-serif">${handle || 'friendscape'}</text>
    <text x="140" y="1550" fill="white" font-size="92" font-family="Arial, sans-serif" font-weight="800">${title}</text>
    <text x="140" y="1640" fill="rgba(255,255,255,0.80)" font-size="44" font-family="Arial, sans-serif">Новый момент готов к просмотру</text>
  </svg>`;
...
function buildStoryMediaFallback(story) {
  if (!story) return '';
  const kind = normalizeKind(story.kind);
  if (kind === 'text') return '';
  return buildSeedMediaDataUri({
    firstName: story.authorLabel || story.author?.name || 'Пользователь',
    publicProfile: { handle: story.authorHandle || story.author?.handle || '' },
  }, 0, kind);
}

function isDemoStory(story) {
  if (!story) return false;
  return Boolean(story.isDemo || story.is_demo || story.source === 'seed' || String(story.id || '').startsWith('seed-story-'));
}
...
function isStoryRenderable(story) {
  return Boolean(story) && !isDemoStory(story) && isDisplayableStory(story);
}

function buildSeedStory(user, index = 0) {
  const now = Date.now() - index * 3_600_000;
  const authorLabel = buildAuthorLabel(user);
  const tone = user?.publicProfile?.tone || 'violet';
  const handle = user?.publicProfile?.handle || null;
  return {
    id: `seed-story-${user.id}`,
    itemId: `seed-item-${user.id}`,
    authorId: user.id,
    authorLabel,
    authorHandle: handle,
    authorTone: tone,
    kind: index % 3 === 2 ? 'video' : index % 3 === 1 ? 'text' : 'photo',
    title: index % 3 === 2 ? 'Момент дня' : index % 3 === 1 ? 'Мысль дня' : 'Новый момент',
    subtitle: index % 3 === 2
      ? 'Подготовка к video stories и live viewer flow.'
      : index % 3 === 1
        ? 'Тут позже будут текстовые stories с анимацией и mentions.'
        : 'Foundation для stories viewer, archive и reply в messenger.',
    previewUrl: buildSeedMediaDataUri(user, index, index % 3 === 2 ? 'video' : 'photo'),
    mediaUrl: buildSeedMediaDataUri(user, index, index % 3 === 2 ? 'video' : 'photo'),
    deepLink: `/stories?story=seed-story-${user.id}&user=${user.id}&source=seed&mode=viewer`,
    audience: 'friends',
    source: 'seed',
    isDemo: true,
    archived: false,
...
async function ensureSeedStories(db = prisma) {
  ...
  store.stories = users.map((user, index) => buildSeedStory(user, index));
  store.seeded = true;
}
...
function serializeStory(story, viewerId = null, store = getStore()) {
  ...
  return {
    ...
    preview_url: story.previewUrl || buildStoryMediaFallback(story) || '',
    media_url: story.mediaUrl || story.previewUrl || buildStoryMediaFallback(story) || '',
  ...
  const highlights = [
    {
      ...
      title: 'Close Friends',
      cover_tone: 'mint',
      count: Math.max(1, archive.length),
    },
    {
      id: 'hl-moments',
      title: 'Моменты',
      cover_tone: 'violet',
      count: Math.max(1, Math.ceil(items.length / 2)),
    },
  ];
...
export async function createStoryFoundation(userId, payload = {}, db = prisma) {
  ...
  const subtitle = clip(payload.subtitle || payload.canvasText || payload.description || payload.copy || '', 280)
    || 'Момент опубликован.';
  const previewUrl = clip(payload.preview_url || payload.previewUrl, 500);
  const mediaUrl = clip(payload.media_url || payload.mediaUrl, 500);
...
export async function replyToStoryFoundation(userId, storyId, payload = {}, db = prisma) {
  ...
  if (isDemoStory(story)) {
    throw Object.assign(new Error('На демонстрационный момент нельзя ответить.'), { status: 400 });
  }
  if (Number(story.authorId) === viewer) {
...
export async function toggleStoryReactionFoundation(userId, storyId, payload = {}, db = prisma) {
  ...
  if (!story) throw Object.assign(new Error('Момент не найден.'), { status: 404 });
  if (isDemoStory(story)) {
    throw Object.assign(new Error('Демонстрационный момент нельзя оценить.'), { status: 400 });
  }
  if (Number(story.authorId) === viewer) {
...
export { DEFAULT_DURATION_MINUTES, MAX_EXTENSIONS, normalizeMomentDuration, canExtendMoment, getMomentExtensionsLeft, getMomentTimeLeft, isDemoStory, isStoryRenderable };
