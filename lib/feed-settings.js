...
const TAB_SET = new Set(['friends', 'following', 'global', 'people', 'all', 'communities']);
...
export function serializeFeedSettings(record) {
  ...
  return {
    ...
    show_communities: Boolean(base.showCommunities ?? DEFAULT_SETTINGS.showCommunities),
...
export function sanitizeFeedSettings(input = {}) {
  return {
    ...
    show_communities: typeof input.show_communities === 'boolean' ? input.show_communities : DEFAULT_SETTINGS.showCommunities,
...
export function getVisibleFeedChannels(record) {
  ...
  // Feed v2 фильтрует выбранный источник на клиенте, поэтому API отдаёт весь доступный личный пул.
  return {
    friends: true,
    following: true,
    global: true,
    communities: settings.show_communities,
  };
}
