...
export function serializeUserPreferences(record) {
  ...
  return {
    ...
    community_visibility: normalizeChoice(source.communityVisibility, VISIBILITY_OPTIONS, DEFAULT_USER_PREFERENCES.community_visibility),
...
export async function updateUserPreferences(userId, patch = {}, db = prisma) {
  ...
  const data = {
    ...
    communityVisibility: normalizeChoice(patch.community_visibility, VISIBILITY_OPTIONS, DEFAULT_USER_PREFERENCES.community_visibility),
...
export async function getProfilePrivacyState(viewerId, targetUserId, db = prisma) {
  const viewer = Number(viewerId);
  const target = Number(targetUserId);
  const [preferences, relation] = await Promise.all([
    getUserPreferences(target, db),
    getViewerRelation(viewer, target, db),
  ]);
  return {
    preferences,
    relation,
    canSeeProfile: isVisibilityAllowed(preferences.profile_visibility, relation),
    canSeePhotos: isVisibilityAllowed(preferences.photo_visibility, relation),
    canSeeActivity: isVisibilityAllowed(preferences.activity_visibility, relation),
    canSeeCommunities: isVisibilityAllowed(preferences.community_visibility, relation),
  };
}
