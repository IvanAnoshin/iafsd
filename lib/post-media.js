...
const ROOT_UPLOADS_DIR = path.join(ROOT_PUBLIC_DIR, 'uploads', 'posts');
const INTERNAL_OBJECT_PREFIX = '/api/storage/post/';
...
function assertPostLocalStorageAllowed(config) {
  if (config.storage !== 'local') return;
  if (process.env.NODE_ENV !== 'production') return;
  if (envBool('POST_MEDIA_ALLOW_LOCAL_IN_PRODUCTION', false)) return;
  throw Object.assign(new Error('Локальное хранилище для медиа постов отключено в production. Настрой object storage или явно включи POST_MEDIA_ALLOW_LOCAL_IN_PRODUCTION=true.'), { status: 500, code: 'LOCAL_MEDIA_STORAGE_BLOCKED' });
}

export function getPostMediaConfig() {
  const image = cloneRule(DEFAULT_CONFIG.image);
  const video = cloneRule(DEFAULT_CONFIG.video);
  image.maxBytes = envNumber('POST_MEDIA_IMAGE_MAX_BYTES', image.maxBytes);
  video.maxBytes = envNumber('POST_MEDIA_VIDEO_MAX_BYTES', video.maxBytes);
  const objectStorage = getObjectStorageConfig('POST_MEDIA');
  const storage = normalizeStorage(process.env.POST_MEDIA_STORAGE || objectStorage.provider || 'local');
  return {
    enabled: String(process.env.POST_MEDIA_ENABLED || 'true').trim().toLowerCase() !== 'false',
    storage,
    basePath: '/uploads/posts',
    objectBasePath: 'posts',
    ...
    stripJpegExif: envBool('POST_MEDIA_STRIP_JPEG_EXIF', true),
    previews: {
      enabled: envBool('POST_MEDIA_PREVIEWS_ENABLED', true),
      mode: String(process.env.POST_MEDIA_PREVIEWS_MODE || 'svg-poster').trim().toLowerCase(),
    },
    limits: { image, video },
  };
}

export function serializePostMediaConfig(config = getPostMediaConfig()) {
  return {
...
export function getPostStorageKeyFromUrl(url) {
  ...
  return getObjectStorageKeyFromPublicUrl(raw, getObjectStorageConfig('POST_MEDIA'));
}
...
function resolvePostUploadPathFromUrl(url, userId) {
  const raw = String(url || '').trim();
  if (!raw.startsWith('/uploads/posts/')) return null;
  const relative = raw.replace(/^\/uploads\/posts\//, '');
...
export async function deletePostUpload({ url, userId, storageKey }) {
  const key = storageKey || getPostStorageKeyFromUrl(url);
  if (key) {
...
export async function storePostUpload({ file, userId, kind, metadata = {} }) {
  const config = getPostMediaConfig();
  if (!config.enabled) throw Object.assign(new Error('Загрузка медиа для постов сейчас отключена.'), { status: 503 });
  assertPostLocalStorageAllowed(config);
  ...
  if (!size || size > rule.maxBytes) throw Object.assign(new Error('Файл превышает допустимый размер для поста.'), { status: 400 });

  await enforceMediaQuota({ db: prisma, prefix: 'POST_MEDIA', ownerUserId: userId, surface: 'post', scopeId: userId, bytes: size });
  ...
  if (config.storage === 'local') {
    ...
    publicUrl = path.posix.join('/uploads/posts', ...parts, filename);
    if (config.previews?.enabled && shouldGeneratePreview(normalizedKind, mime)) {
      ...
      const preview = await writeLocalPreview({ rootDir: ROOT_UPLOADS_DIR, basePath: '/uploads/posts', parts, filename, previewBuffer });
      thumbUrl = preview.thumbUrl;
      previewBytes = preview.previewBytes;
    }
  } else if (isObjectStorageMode(config.storage)) {
    storageKey = ['posts', ...parts, filename].join('/');
    const objectConfig = getObjectStorageConfig('POST_MEDIA');
    ...
    if (config.previews?.enabled && shouldGeneratePreview(normalizedKind, mime)) {
      const previewBuffer = buildMediaPreviewSvg({ kind: normalizedKind, mime, originalName });
      previewStorageKey = buildObjectPreviewKey({ rootPrefix: 'posts', parts, filename });
      const objectConfigForPreview = getObjectStorageConfig('POST_MEDIA');
    ...
    throw Object.assign(new Error(`Неподдерживаемое post media storage: ${config.storage}`), { status: 500 });
  }
  ...
  await recordMediaObject({
    ...
    bytes: buffer.length,
    previewBytes,
    metadata: { originalName, exifStripped: stripped, private: isObjectStorageMode(config.storage) ? getObjectStorageConfig('POST_MEDIA').privateAccess : false },
  });
  ...
  return {
    ...
    private: isObjectStorageMode(config.storage) ? getObjectStorageConfig('POST_MEDIA').privateAccess : false,
    mime,
    detectedMime: security.detectedMime,
    bytes: buffer.length,
    originalBytes: size,
